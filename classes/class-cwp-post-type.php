...
abstract class CWP_Post_Type {
	...
	public function init( $priority = 11 ){
		...
		if ( ! empty( $fields['_redirect'] ) ){
			
			add_filter( 'post_type_link', array( $this , 'set_redirect' ), 10 , 2 );
			
		} // end if
		
		if ( $this->get_admin_enqueue() ){
	...
	/**
	 * Get method for admin_enqueue
	 * @return string|array
	 */
	public function get_admin_enqueue() { return $this->admin_enqueue; }
	...
	/**
	 * Add admin scripts to custom post type
	...
	public function add_admin_scripts( $hook ){
		...
		if ( $hook == 'post-new.php' || $hook == 'post.php' ) {
			...
			if ( 'recipes' === $post->post_type ) {
				...
				foreach( $this->get_admin_enqueue() as $index => $enqueue ){
					
					if ( $enqueue['type'] == 'script' ){
						
						wp_enqueue_script( $enqueue['handle'], plugins_url( $enqueue['url'] , dirname(__FILE__) ) , $enqueue['deps'] , $enqueue['ver'] , $enqueue['in_footer'] );
	 ...
	 */
	public function set_redirect( $url , $post ){
